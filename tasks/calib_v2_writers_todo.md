...
- [x] **Item 7**: Per-MoE-block output (128-prompt subset) writer + Stage 3 `block_refine` cache reader. **Stage 2.5 `merge_repair` reader SCOPE-CUT** — router_kd trainer uses its own dataloader (not calibration JSONL); aligning the writer's flat sidecar with the trainer's per-batch `input_ids` is intractable without a router_kd refactor. The Stage 3 reader captures the high-cost win (skip live teacher block forward inside Phase C.5's `_phase_c5_block_refine`). See plan doc "Scope-cut consumer notes". New tag candidate `calib-v2-block-outputs-writer`.
