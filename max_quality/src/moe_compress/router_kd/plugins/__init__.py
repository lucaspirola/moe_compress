...
the universal fallback) and ``merge_repair.py`` (added by RK-6 — the
...
stage-2.5-gated ``is_enabled``). The unified Router-KD algorithm — the KD
training loop serving both Stage 2.5 and Stage 5 — is extracted from the
legacy ``stage5_router_kd.py`` monolith into focused plugins here; the
remaining Router-KD plugins land in this package by task RK-7.
"""
