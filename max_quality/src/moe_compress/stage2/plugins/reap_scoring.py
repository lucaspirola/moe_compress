...
- **D11 — calibration data source**: REAP (2510.13999 §5) uses
  ...
  in the run YAML — it has changed over the project's life
  (Nemotron-Cascade-2-SFT-Data → Tülu-3 SFT-mix →
  qwen3-pretrain-mix) and may change again. Task-aware calibration
  better matches target deployment distribution; the specific corpus
  and any subset weights are config-driven, not baked in here.
- **D-cal-size — calibration sequence count**: REAM 2604.04356 §5 uses
  ...
  ``config["calibration"]["sequence_length"]`` tokens. The
  longer-of-the-two paper sequence lengths (2048, matching REAP) is
  chosen to match the deployment context length; the count is
