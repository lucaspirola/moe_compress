...
``partial_dir``) lives as instance attributes on this adapter — ``RunContext``
stays frozen and ``extras``-less. The adapter is constructed once per
``run()`` invocation, so the per-adapter scratchpad is single-run-scoped with
no concurrency hazard. Per-layer scratchpad lives on ``LayerContext`` (typed
slots) and ``ctx.extras`` (free-form dict for fields not yet promoted to
typed slots).
"""
...
from .._framework.context import LayerContext, RunContext
...
class LegacyAdapter(Stage2Plugin):
    ...
    def __init__(
        ...
        *,
        ...
        # Held here on the adapter instance because RunContext is frozen and
        # explicitly does NOT carry an ``extras`` dict (T1 design invariant).
        # In-place mutations on these references are visible to run() after
        # the per-layer loop exits.
    ...
    def on_layer_setup(self, ctx: LayerContext) -> None:
        ...
        ctx.ream_acc = ream_acc
        ctx.perm_cache = perm_cache
        ctx.layer_input_acc = layer_input_acc
    ...
    def on_profile(self, ctx: LayerContext) -> None:
        ...
        layer_ref = ctx.layer_ref
        _srr._profile_layer(
            self.model, layer_ref, self.batches, ctx.reap_acc, self.cov_acc, ctx.ream_acc,
            device=self.device,
            layer_input_acc=ctx.layer_input_acc,
        )
    ...
    def compute_assignment(self, ctx: LayerContext) -> None:
        ...
        layer_ref = ctx.layer_ref
        reap_acc = ctx.reap_acc
        ream_acc = ctx.ream_acc
        perm_cache = ctx.perm_cache
        layer_input_acc = ctx.layer_input_acc
        ...
        # Surface bump-loop outputs on ctx for downstream phases.
        ctx.protected = tuple(sorted(protected))
        ctx.scores = scores
        ctx.freq = freq
        ctx.ream_centroid_ids = tuple(ream_centroid_ids)
        ctx.ream_noncentroid_ids = tuple(ream_noncentroid_ids)
        ctx.assignment = assignment
        ctx.delta = delta
        ctx.grouped = grouped
        ctx.mean_assigned_cost = mean_assigned_cost
        ctx.extras["n_protected"] = n_protected
        ctx.extras["assigned_cost"] = assigned_cost
        ctx.extras["n_assigned"] = n_assigned
        ctx.extras["b_fail"] = b_fail
        ctx.extras["c_fail"] = c_fail
        ctx.extras["em_rounds_done"] = em_rounds_done
        ctx.extras["effective_cost_alignment"] = effective_cost_alignment
        ctx.extras["effective_cost_asymmetric"] = effective_cost_asymmetric
        ctx.extras["capacity_util_value"] = capacity_util_value
        ctx.extras["effective_target"] = effective_target
    ...
    def pre_merge_snapshot(self, ctx: LayerContext) -> None:
        ...
        ctx.extras["nemo_writer"] = nemo_writer
        ctx.extras["xd_writer"] = xd_writer
        ctx.extras["pre_merge_weights"] = pre_merge_weights
        ctx.extras["layer_merged"] = layer_merged
    ...
    def merge(self, ctx: LayerContext) -> None:
        ...
        perm_cache = ctx.perm_cache
        layer_input_acc = ctx.layer_input_acc
        pre_merge_weights = ctx.extras["pre_merge_weights"]
        ...
        ctx.distill_state = distill_state
    ...
    def post_merge(self, ctx: LayerContext) -> None:
        ...
        layer_ref = ctx.layer_ref
        protected = list(ctx.protected)
        ream_centroid_ids = list(ctx.ream_centroid_ids)
        nemo_writer: ShardWriter | None = ctx.extras["nemo_writer"]
        xd_writer: ShardWriter | None = ctx.extras["xd_writer"]
        ...
        ctx.final_kept_ids = tuple(final_kept_ids)
        ctx.heal_state = heal_state
    ...
    def write_artifacts(self, ctx: LayerContext, partial_dir: Any) -> dict[str, Any]:
        ...
        grouped = ctx.grouped
        freq = ctx.freq
        final_kept_ids = list(ctx.final_kept_ids)
        ream_centroid_ids = list(ctx.ream_centroid_ids)
        ream_acc = ctx.ream_acc
        merge_map = self.merge_map
        cov_acc = self.cov_acc
        heal_state = ctx.heal_state
        distill_state = ctx.distill_state
        # Read bump-loop outputs from extras (typed-slot promotion happens in
        # later tasks).
        n_experts = ctx.n_experts
        n_protected = ctx.extras["n_protected"]
        assigned_cost = ctx.extras["assigned_cost"]
        n_assigned = ctx.extras["n_assigned"]
        c_fail = ctx.extras["c_fail"]
        em_rounds_done = ctx.extras["em_rounds_done"]
        effective_cost_alignment = ctx.extras["effective_cost_alignment"]
        effective_cost_asymmetric = ctx.extras["effective_cost_asymmetric"]
        capacity_util_value = ctx.extras["capacity_util_value"]
        effective_target = ctx.extras["effective_target"]
        mean_assigned_cost = ctx.mean_assigned_cost if ctx.mean_assigned_cost is not None else 0.0
        ...
        _trackio_log({
            ...
            "stage2/stage1_target": ctx.target,
    ...
    def on_layer_teardown(self, ctx: LayerContext) -> None:
        ...
        ctx.ream_acc = None
        ctx.perm_cache = None
        if ctx.layer_input_acc is not None:
            ctx.layer_input_acc = None
        if ctx.extras.get("pre_merge_weights") is not None:
            ctx.extras["pre_merge_weights"] = None
        if ctx.distill_state is not None:
            ctx.distill_state = None
        # Drop large transient writers / accumulators from extras as well so
        # gc.collect() can reclaim their underlying buffers immediately.
        ctx.extras.pop("nemo_writer", None)
        ctx.extras.pop("xd_writer", None)
        gc.collect()
        torch.cuda.empty_cache()
