...
def _wikitext2_ppl(model, tokenizer, cfg: dict, *, device=None, collect=None,
    ...
    * ``prebuilt`` (in): when a dict carrying ``{"tokenizer": ..., "chunks":
    ...
    # DP eval-shard branch (M3 — gated by eval_shard.cfg.ppl SEPARATELY from the
    # gen path; default OFF → in-process loop VERBATIM). PPL is BATCH_INVARIANT,
    # so a row shard is exact regardless of boundary and each replica may size
    # its own forward batch; run_dp_ppl sums the (nll_sum, tok_count) partials
    # to the identical global PPL.
    if eval_shard is not None and eval_shard["cfg"].ppl:
        from ...tools.eval_shard import _should_shard as _ss, run_dp_ppl
        _dp_replicas = _ss(eval_shard["cfg"], int(chunks.shape[0]), group_aligned=False)
        if _dp_replicas > 1:
            return run_dp_ppl(
                chunks,
                ...
                auto_batch_cfg_dict=eval_shard.get("auto_batch_cfg_dict"),
            )
        ...
        try:
            ...
            if _es_cfg.enabled and _es_cfg.replicas > 1 and ctx.has("eval_shard_src_dir"):
                ...
                _eval_shard = {
                    ...
                    "out_dir": str(_artifacts_dir) + "/_eval_shard_wikitext2",
                }
        except Exception:  # noqa: BLE001
            _eval_shard = None
