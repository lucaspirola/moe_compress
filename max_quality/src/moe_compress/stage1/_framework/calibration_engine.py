"""Cross-stage Phase-B calibration-pass engine.

This module exposes :class:`CalibrationEngine` — a declarative wrapper around
the existing primitives in :mod:`moe_compress.utils.activation_hooks`. Stage-1
detector plugins (sub-tasks 6-9) and Stage-2 detectors register their
...
The engine is stage-agnostic. It is a verbatim extraction of the legacy
Stage 1 inline calibration-pass plumbing (Phase B), now the single source
of truth for that profiling pass.

Public surface (import directly from
``moe_compress.stage1._framework.calibration_engine``)::
