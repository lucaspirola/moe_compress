...
* ``calibration_corpus.py`` (added by S6A-2 — the thermometer calibration-
  corpus concern: the relocated ``_thermo_corpus_spec`` /
  ``_thermo_wikitext_tensor`` / ``_build_thermo_corpus`` helpers plus a
  ``CalibrationCorpusPlugin`` with an unconditional ``is_enabled`` and an
...
The Stage 6alt thermometer algorithm — calibration-corpus build, teacher-
cache provider, BPT measurement, lm-eval subset, and validation-report
assembly — is extracted from the legacy ``stage6alt_thermometer.py``
monolith into focused plugins here by tasks S6A-2..S6A-5. No plugin
manifest exists yet — S6A-6 introduces the ``STAGE6ALT`` object and flips
the orchestrator-vs-monolith delegation direction.
"""
