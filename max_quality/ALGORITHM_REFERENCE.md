...
**Hardware:** H200 (141 GB). Pruned student stays resident from Stage 2.5. Original BF16 teacher is loaded for the Phase A cross-covariance dual-forward (~120 GB combined) and **stays resident through Phase C.5** so its block forwards can be invoked on-demand during block refinement. Phase C.5 trains one block at a time; per-block trainables (~1.25 GB BF16) plus AdamW state + activations + grads (~10–15 GB) fit in the remaining ~20 GB headroom. The previous spec variant freed the teacher after Phase A and fed Phase C.5 from a 215 GB on-disk teacher-block-output cache (Phase A.X) — that path was an A100-era constraint and is **removed** on H200.
...
**Teacher activation stream:** At block `i`, `X_i^{teacher}` is the cumulative teacher upstream — the teacher's hidden state at block `i`'s input, produced by running the teacher from layer 0 through layer `i−1` on the calibration sequence. Per gradient step, both `X_i^{teacher}` (input to teacher block `i`) and `X'_i^{student}` (input to refined student block `i`, produced by the already-refined upstream student blocks) are computed. The teacher upstream stream is computed once per calibration batch and reused across the 25 epochs for that block — it does not change during the AdamW optimization of block `i`. To avoid recomputing the teacher's full upstream stream from layer 0 every step, the teacher's per-block outputs `X_{i+1}^{teacher} = ℒ_i(X_i^{teacher})` are computed sequentially as Phase C.5 progresses block-by-block, with the previous block's teacher output kept in CPU RAM as the starting point for the next block (~2 GB per layer × 1 = 2 GB resident, swapped per block).
