...
- **All subsequent layers (dual-signal OR rule)**: add to `L` if **either** of the following two growth ratios exceeds its threshold:
  ...
  - **MoE-output growth** (secondary, ungated): `max|MoE_l(x)| / max|MoE_{l-1}(x)| > moe_output_growth_ratio`
    where `MoE_l` is the SparseMoeBlock output (post-routing-weighted-sum, pre-residual-add). The MoE branch is **not** dampened by `attn_output_gate` (the gate is only on the attention branch), so this signal carries the raw MA-injection magnitude unattenuated. **`moe_output_growth_ratio = 2.0`** project-specified — calibrated to be slightly more permissive than the gated residual signal because MoE-output magnitudes naturally grow as the residual accumulates.
  - The **OR rule** maximizes recall: a layer is added to `L` if it forms an MA visible in either branch. Rationale (reviewer's exact recommendation): residual-stream growth alone misses MoE-mediated MA injection events that are dampened by sigmoid gate at the attention output; MoE-output growth alone misses attention-mediated MA formation in ungated architectures. Carrying both signals covers both architectures.
