...
| D-aimer-cross-check | 1 | AIMER weight-only auto-extension to SE blacklist | None — AIMER (arxiv:2603.18492) is a calibration-free pruning heuristic, not part of the Super Experts paper | After Phase C, compute per-expert AIMER `\|\|w\|\|_1 / (sqrt(N)·\|\|w\|\|_2)` on `down_proj` weights. Auto-add `(l, e)` to blacklist if AIMER bottom-`aimer_bottom_pct` (=1%) AND layer's expert-level max > `aimer_layer_max_fraction · a_max_absolute` (=0.1·a_max) AND not already blacklisted. Provenance tagged "aimer". | Reviewer (`max_quality/docs/stage1_review_10.05.2026.txt`) recommends AIMER as a complementary signal independent of residual-stream dynamics. Auto-extension (vs report-only) chosen by user after explicit scope confirmation. Conditions (2) and (3) bound risk: AIMER cannot extend cold layers or duplicate Phase C decisions. |
| D-sink-token-routing | 1 | Sink-token routing-pattern auto-extension to SE blacklist | None — sink-token analysis is mentioned in 2507.23279 Figure 20-21 appendix as descriptive observation, not detection criterion | Hook each layer's router output during Phase B; aggregate per (layer, expert) the `mean_router_score_on_sink_tokens`, `mean_router_score_on_normal_tokens`, and `activation_frequency_on_sink_tokens`. Sink tokens = positions where input_id == BOS_token_id ∪ position 0 of each sequence. **Per-layer normalization**: `freq_on_sink[(l, e)] = sink_fires[(l, e)] / total_sink_tokens_seen_by_layer[l]` — the denominator is per-layer (all layers see identical calibration data so the counts coincide, but the per-layer contract eliminates the ambiguity that previously caused a num_layers-fold double-count). Auto-add `(l, e)` to blacklist if `score_sink / max(score_normal, ε) > sink_token_score_ratio` (=5.0) AND `freq_on_sink > sink_token_freq_threshold` (=0.95) AND not already blacklisted. Provenance tagged "sink_token". Implementation: vectorized per-batch reduction (per-layer arrays of shape `(num_experts,)` populated via `torch.nn.functional.one_hot` on the routed indices) — replaced an earlier per-expert Python loop that was the dominant Phase B walltime cost on H200 (~5 sec/batch). | Architecturally independent of residual-stream magnitudes; flags experts whose routing pattern is sink-token-dominated (the paper's Appendix Figures show SEs concentrate on sink tokens). Same risk-floor logic as AIMER: requires both score-ratio AND freq-threshold to fire, plus de-duplication against Phase C. The vectorization additionally fixed a num_layers-fold double-count in `freq_on_sink` (artifact from 2026-05-10 H200 run had max `freq_on_sink = 1/40 = 0.025` instead of 1.0, leaving the 0.95 threshold unreachable). |
| D-causal-ablation-validation | 1 | Phase F post-hoc causal-ablation report | None — paper validates SE detection via global ablations (Table 3) but not as a per-expert validation step | After Phase C + auto-extensions, run single-expert ablation forward passes on a held-out 100-sample slice. Report ΔNLL (mean per-token cross-entropy difference) for every blacklisted expert + top-5 non-blacklisted candidates per `l ∈ L`. Report-only — operator decides whether to manually extend. Output: `stage1_post_hoc_ablation.json`. | Reviewer's recommended validation step. Cost (~30 min on H200) is small vs Stage 1's total walltime; the report lets us catch missed SEs before downstream ablations spend GPU on a wrong blacklist. |
