...
**Vectorized accumulators (compute-time optimization, zero quality impact):**

The REAM cost matrix computation involves two pairwise similarity metrics across all experts in a layer (up to 256 experts). The accumulators use dense tensors instead of Python dicts for O(1) vectorized operations:
