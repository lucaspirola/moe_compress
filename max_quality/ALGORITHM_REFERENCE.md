...
**Algorithm (Algorithm 1, lines 4–12 — condensed rendering; line 3 is the section header) (spec drops the inner pseudocode at paper line 8 'if MA pattern detected' which the paper leaves undefined; spec replaces it with the explicit dynamic-detector formula in D-ma-detector):**
...
- **First MoE layer** (no predecessor to compare; dense pre-MoE layers in the stack are not MoE candidates and are skipped automatically; the "first" here is the first MoE layer encountered top-down): absolute outlier check — add to `L` if `max|H_l(x)| > ma_ratio × Q_99(|H_l(x)|)` where the LHS is the maximum-magnitude scalar across all calibration tokens (taken as a running max across batches) and the RHS is the **99th percentile across all token magnitudes seen** during Phase A (computed exactly by buffering per-batch flattened magnitudes and calling `np.percentile(concat, 99.0)` once at finalize time per [D-ma-detector]). **`ma_ratio = 100` is project-specified** (not from the paper). If the first MoE layer fails its absolute-outlier check (max|H_l(x)| ≤ 100·Q_99(...)), it is not added to L by Phase A; it can only enter L via the 0.75-depth fallback if the dynamic detector returns ∅ globally.
- **All subsequent layers (dual-signal OR rule)**: add to `L` if **either** of the following two growth ratios exceeds its threshold:
  - **Residual-stream growth** (primary, gated): `max|H_l(x)| / max|H_{l-1}(x)| > ma_growth_ratio`
    where `H_l` is the post-decoder-layer residual stream. **`ma_growth_ratio = 3.0`** for Qwen3.5/3.6-class architectures with `attn_output_gate=true` (project-specified, motivated by `5.0 × E[sigmoid(gate)] ≈ 5.0 × 0.6 = 3.0`; see [D-ma-detector](#12-known-deviations-from-papers)). For un-gated architectures (LLaMA, Mixtral, Qwen3-30B), the paper's `5.0` calibration applies.
  - **MoE-output growth** (secondary, ungated): `max|MoE_l(x)| / max|MoE_{l-1}(x)| > moe_output_growth_ratio`
    where `MoE_l` is the SparseMoeBlock output (post-routing-weighted-sum, pre-residual-add). The MoE branch is **not** dampened by `attn_output_gate` (the gate is only on the attention branch), so this signal carries the raw MA-injection magnitude unattenuated. **`moe_output_growth_ratio = 2.0`** is a project choice on top of the reviewer's "respective threshold" framing — the reviewer recommended dual-signal flagging but did not prescribe a specific MoE-branch threshold. The `2.0` value was tuned empirically: lower than `ma_growth_ratio` because the ungated MoE-output signal has a smaller dynamic range than the gated-attention-plus-MoE residual stream; calibrated to maximize recall under the OR rule without picking up noise.
  - The **OR rule** maximizes recall: a layer is added to `L` if it forms an MA visible in either branch. Rationale: residual-stream growth catches attention-mediated MA-formation events (the attention branch contributes only to the residual, not to the MoE block output); MoE-output growth catches MoE-mediated MA-injection events (visible in the ungated MoE branch even when the gated residual contribution is dampened, as in Qwen3.5/3.6's `attn_output_gate=true`). Carrying both signals covers both MA-formation pathways across gated and ungated architectures.
  - **Architectural note (Qwen3.5/3.6)**: full-attention layers (1 of every 4) apply `attn_output_gate=true` (sigmoid gate on attn output before residual add); the other 3-of-4 linear-attention layers apply `FusedRMSNormGated` (combined RMS-norm + sigmoid gate inside the linear-attention path), which makes the attention-branch contribution magnitude-flat regardless of input. Consequence: at linear-attention layers, residual-stream growth is driven exclusively by the MoE branch — the MoE-output signal is the only meaningful detector there, while the residual-stream signal is informative only at full-attention layers (the MA-attention-sink feedback loop modified by gated attention is analyzed in arxiv:2603.17771).
...
**Note on full-attention "reset" layers (Qwen3.6-specific):** Qwen3.6's `[linear, linear, linear, full]` layer-type cycle places full-attention layers at indices {3, 7, 11, 15, 19, 23, 27, 31, 35, 39}. The 8th full-attention layer (L31 in Qwen3.6) can show an aggressive residual-stream **collapse** (e.g., 42.5 → 6.97 in our diagnostic, 0.16× ratio). This is **expected behavior** (see arxiv:2508.03616 for cross-architecture MA reset dynamics): the gated attention branch (`attn_output = attn_raw · sigmoid(gate)`) can learn near-zero gate logits in MA-carrying feature dimensions, selectively suppressing the MA channel. The collapse is mechanistically a learned attention-mediated denoising of the residual, **not** an MoE event — the layer's MoE branch operates normally and its experts should be subject to the same GRAPE/REAM treatment as any other layer. Phase A correctly ignores collapses (it flags growth, not drops). Verification hooks for this hypothesis live at `max_quality/scripts/l31_verification.py`, which captures three signals at L31: (i) the pre-residual attention contribution via `o_proj` output, (ii) `sigmoid(gate)` activations from the q_proj output (split via the standard view-and-chunk on the per-head `head_dim*2` axis), and (iii) the MoE block output. Mean magnitudes are partitioned by MA-carrying vs non-MA-carrying feature dimensions (where MA dimensions are identified at L30 by top-K mean abs residual magnitude); the gate-suppression hypothesis predicts the MA/non-MA ratio is ≪ 1 for both `sigmoid(gate)` and the post-projection attention output, while the MoE ratio is ≈ 1.
...
|----|-------|-----------|-----------|-------------------|---------------|
| D-SE-A | 1 | SE detection: A restricted to l∈L | 2507.23279 §3.2.1 prose: "across the entire model"; Algorithm 1 (Stage 2 block, lines 14–32): restricted to l ∈ L at the layer-loop header on line 16 | A is restricted to l ∈ L (MA-formation layers only) per Algorithm 1 (Stage 2 block, lines 14–32); the `l ∈ L` restriction is enforced at the layer-loop header on line 16 | The paper's Algorithm 1 is the more precise procedural definition (the `l ∈ L` restriction at the layer loop is unambiguous). The §3.2.1 prose ("across the entire model") is imprecise and contradicted by the pseudocode. (External validation: the authors' official implementation at github.com/ZunhaiSu/Super-Experts-Profilling matches Algorithm 1 — not auditable from this repo's local artifacts.) |
...
| D-ma-detector | 1 | Phase A MA-formation detector thresholds + sampling caps | 2507.23279 Algorithm 1 line 8: "if MA pattern detected" with no formula; the official implementation (github.com/ZunhaiSu/Super-Experts-Profilling, `_super_experts_analysis`) uses a fixed `include_layers=0.75` depth heuristic instead of dynamic detection; CKA reservoir-sample size is not specified by GRAPE §3.2 | Project-specified Stage 1 sampling/threshold choices (Qwen3.5/3.6 calibration; un-gated architectures retain paper defaults): (1) **dynamic detector primary** — `ma_ratio = 100` (first MoE layer, max\|H_l\| > ma_ratio · Q99(\|H_l\|)), `ma_growth_ratio = 3.0` (subsequent layers, residual stream), `moe_output_growth_ratio = 2.0` (subsequent layers, MoE block output, OR'd with residual). (2) **0.75-depth fallback secondary** — only when the dynamic detector returns ∅, fall back to `layer_index < round(0.75 × total_layers)`. (3) **CKA reservoir cap = 256 tokens per expert** for Phase E. (4) **Per-phase batch sizes**: `phase_a_batch_size = 32`, `phase_b_batch_size = 8`, `ablation_filter.batch_size = 32`. All Phase B accumulators (`DownProjMaxAccumulator`, `ExpertOutputAccumulator` reservoir, `SinkTokenRoutingAccumulator` vectorized reduction) handle arbitrary `B` — the live 2026-05-10 H200 run at bs=1 used 94.6/150.8 GB VRAM (37% free, ~25 GB non-model state), so bs=8 yields ~120-130 GB total with ~20-30 GB headroom. Phase D's held-out NLL forwards are also batch-size invariant; bs=32 cuts each ablation's batch count from 100 to ~3 (verify on first run; drop to 16 if VRAM near 145 GB). (5) **num_calibration_samples = 1024** (down from 4000) — saturates layer-max and the 256-token reservoir at low cost. | Paper gives no detector formula. The 3.0 / 2.0 thresholds are calibrated for `attn_output_gate=true` (Qwen3.5/3.6): `5.0 × E[sigmoid(gate)] ≈ 5.0 × 0.6 = 3.0` for the gated residual stream; the MoE branch is ungated and uses a separate, slightly more permissive threshold to maximize recall under the OR rule. The 0.75-depth fallback ensures `L` is never empty. Reservoir cap balances kernel conditioning vs cost; sample count reduction is reviewer-recommended (Frobenius distance of CKA matrices changes < 5% between 1024 and 4000 samples per the AIMER paper's calibration sensitivity analysis). Reviewer report `max_quality/docs/stage1_review_10.05.2026.txt` documents these decisions in detail. |
...
| D-aimer-cross-check | 1 | AIMER candidate-pool contributor (final blacklist gated by Phase D ablation) | None — AIMER (arxiv:2603.18492) is a calibration-free pruning heuristic, not part of the Super Experts paper | Phase C uses AIMER as a candidate source: per-expert AIMER `\|\|w\|\|_1 / (sqrt(N)·\|\|w\|\|_2)` on `down_proj` weights. The bottom-`aimer_bottom_pct` (=1%) per layer enters the Phase C candidate pool, gated by `per_expert_max[l] > aimer_layer_max_fraction · a_max`. Provenance tagged "aimer". Final inclusion in `stage1_blacklist.json` requires Phase D ablation evidence (`ΔNLL > ablation_filter_threshold`); v5's "auto-extension to blacklist" is removed. | Reviewer (`max_quality/docs/stage1_review_10.05.2026.txt`) recommends AIMER as a complementary signal independent of residual-stream dynamics. v6 demoted AIMER from auto-extension to candidate-only because static-threshold detection is fragile across architectures (cf. D-sink-token-routing v4 false-positive rate); ablation evidence is the only ground-truth filter. The bottom-pct + cold-layer gate keep candidate-pool size small. |
| D-sink-token-routing | 1 | Sink-token routing-pattern candidate-pool contributor (tightened thresholds + per-layer cap; final blacklist gated by Phase D) | None — sink-token analysis is mentioned in 2507.23279 Figure 20-21 appendix as descriptive observation, not detection criterion | Hook each layer's router output during Phase B; aggregate per (layer, expert) the `mean_router_score_on_sink_tokens`, `mean_router_score_on_normal_tokens`, and `activation_frequency_on_sink_tokens`. Sink tokens = positions where input_id == BOS_token_id ∪ position 0 of each sequence. **Per-layer normalization**: `freq_on_sink[(l, e)] = sink_fires[(l, e)] / total_sink_tokens_seen_by_layer[l]` — the denominator is per-layer (all layers see identical calibration data so the counts coincide, but the per-layer contract eliminates the ambiguity that previously caused a num_layers-fold double-count). Phase C adds `(l, e)` to the candidate pool if `score_sink / max(score_normal, ε) > sink_token_score_ratio` (=10.0; tightened from v4's 5.0) AND `freq_on_sink > sink_token_freq_threshold` (=0.99; tightened from v4's 0.95) AND not exceeding `sink_token_max_per_layer_cap` (=10) per layer (sorted by score_ratio descending). Provenance tagged "sink_token". Final blacklist requires Phase D ablation evidence. Implementation: vectorized per-batch reduction (per-layer arrays of shape `(num_experts,)` populated via `torch.nn.functional.one_hot` on the routed indices) — replaced an earlier per-expert Python loop that was the dominant Phase B walltime cost on H200 (~5 sec/batch). | Architecturally independent of residual-stream magnitudes; flags experts whose routing pattern is sink-token-dominated (the paper's Appendix Figures show SEs concentrate on sink tokens). v6 demoted from auto-extension to candidate-only and tightened thresholds because v4's `freq_on_sink` saturated at 1.0 for many non-SE experts on Qwen3.6 (146 of v4's 158-expert blacklist came from this detector but only 1 had measurable Phase F ΔNLL — sink-token routing is broadly distributed on this architecture rather than SE-specific). The per-layer cap bounds candidate-set size on architectures where the score-ratio + freq criteria still over-fire after tightening. The vectorization additionally fixed a num_layers-fold double-count in `freq_on_sink` (artifact from 2026-05-10 H200 run had max `freq_on_sink = 1/40 = 0.025` instead of 1.0, leaving the 0.95 threshold unreachable). |
| D-causal-ablation-validation | 1 | Phase D ablation filter (project-original; load-bearing — produces final blacklist) | None — paper validates SE detection via global ablations (Table 3) but not as a per-expert filter | Phase D ablates every Phase C candidate over a held-out 100-sample slice (deterministic seed offset distinct from Phase A/B; cached at `_calibration_cache_phase_d/`). For each `(l, e)` candidate, install a forward hook that zeros `down_proj` output, measure ΔNLL = ablated_nll − baseline_nll, and include `(l, e)` in the final blacklist iff `ΔNLL > ablation_filter_threshold` (=0.001 ≈ 0.1% PPL impact). Per-candidate ΔNLL retained in `stage1_ablation_filter.json` for audit. Default `ablation_filter_batch_size = 32` over 100 samples (~4 batches/candidate) → ~10–17 min for 60–100 candidates on H200. | v4 produced a 158-expert blacklist of which only 5 had measurable ΔNLL; 144 were dead-weight, 9 were active false positives that *hurt* PPL when protected. Static-threshold detection is fragile across architectures (each new architecture shifts the right thresholds and the right values are unknown until the model is run); ablation evidence is ground truth. v6 promoted ablation from report-only to the load-bearing final filter and rewrote Phase C as a candidate-pool generator gated by Phase D evidence. |
...
| D-a-max-fraction | 1 | `a_max_fraction` configurable knob | 2507.23279 Eq. 6: `a_{l,e} > 0.1·a_max` with the 0.1 multiplier as a fixed paper constant | Configurable via `stage1_grape.super_expert_detection.a_max_fraction` (default 0.1; production config sets 0.1) | Project-original tunable that the paper does not contemplate; default value matches the paper exactly. The knob exists so an operator can sweep ablations on the second SE-criterion threshold without code changes; production runs MUST keep `a_max_fraction=0.1` for paper-compliant SE detection. |
