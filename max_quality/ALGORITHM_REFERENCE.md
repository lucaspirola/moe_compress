...
**Official implementation note:** The authors' released code (github.com/ZunhaiSu/Super-Experts-Profilling) does not implement dynamic MA-formation detection. Instead it uses a fixed depth heuristic: restrict to layers with index ≥ `round(0.75 × total_layers)` (`include_layers=0.75` parameter, default). This spec's Phase A (dynamically detecting MA-formation layers via the hidden-state outlier check) is more faithful to Algorithm 1's intent but produces a different set L than the 75%-depth cutoff would on Qwen3.6-35B-A3B. For Qwen3-30B-A3B the paper reports L = {1, 2, 3} (the first three layers), which aligns with early-layer MA formation rather than the last 75% of layers — suggesting the authors' heuristic and Algorithm 1 capture different populations on this model, and the dynamic detection is the more principled choice.
