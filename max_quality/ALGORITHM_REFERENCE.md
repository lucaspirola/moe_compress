...
Using across-batch maxima is valid because MAs are input-stable (paper §3.2.2 (Algorithm 1 section), lines 405–406: "their distribution remains stable across different input datasets").
...
2. **Greedy loop** until total surviving experts ≤ `global_expert_budget`:
   - If all layers frozen and budget not yet met → **restart** (unfreeze all; if budget already met, the outer loop exits normally). The same iteration that clears `frozen` then immediately runs argmin layer/pair selection and performs one merge before the entropy gate is re-evaluated next iteration — this "one extra merge per restart cycle to escape local optima" is documented at [D-grape-restart-merge](#12-known-deviations-from-papers).
...
R̃^l = (R^l − min R) / (max R − min R)           (Eq. 3 — for logging only; under the distance convention from [D-cka-distance], the explicit identity is R̃^l_dist = 1 − R̃^l_sim — the constant offset N(N−1) cancels from both numerator and denominator after min-max normalization, so polarity is inverted vs paper Eq. 3 but the ranking is preserved)
...
**Teacher loading fallbacks:** Stage 2.5 may use the same 4-bit / cache fallbacks as Stage 5 if VRAM is tight — teacher loadable in 4-bit via bitsandbytes, or precomputed teacher vocabulary logits loaded from a sidecar cache file (`teacher_logits_cache` config key) to skip the live teacher entirely (cache wins on conflict — see §8 Teacher Loading for the precedence rule). See §8 "Teacher Loading" for details. (No Phase C.5–style RMSNorm carve-out applies at Stage 2.5/5; that exception is scoped to Stage 3 only, per §10.)
...
**Phase A.X: Teacher block-output cache for Phase C.5.** Phase C.5's anchored objective `ℓ_i = E[‖ℒ_i(X) − ℒ'_i(X')‖²]` requires `ℒ_i(X)` — the *original* (unfactorized) block's output on the calibration activations `X` arriving at block `i` from the teacher. The Stage 4 originals snapshot (`_stage3_original_weights.pt`, see Resume) saves expert weights only and cannot be invoked as a whole-block forward without paying the full teacher load again at Phase C.5 time. To preserve VRAM economy (avoid keeping the teacher resident through Phase C.5), the dual-forward in Phase A is extended: while the teacher is still resident, the per-block teacher output `ℒ_i(X_teacher)` is captured for every calibration batch at every decoder layer `i ∈ {0, …, N_layers−1}` and spilled to disk under `_stage3_teacher_block_out_partial/layer_{i}.pt` via the same background I/O thread that handles B/C spills. Phase C.5 then memory-maps the spill for the block currently under refinement; only one block's tensor is resident at a time. (a) The cache stores the teacher block's output in **BF16** (matching teacher precision) to balance fidelity vs disk; (b) the cached tensor at layer `i` is `ℒ_i(X_i^{teacher})` — the teacher block's output on the **teacher's own upstream activation stream** (cumulative teacher upstream from block 0 through `i−1`), NOT on the shifted student input `X'`. This anchors the Phase C.5 objective `‖ℒ_i(X) − ℒ'_i(X')‖²` correctly: `ℒ_i(X)` is the cached teacher output. (`torch.load(..., mmap=True)` requires PyTorch ≥ 2.1; project pins ≥ 2.11.)

**Disk footprint:** approximately `N_calib_seqs × seq_len × hidden_size × 2 bytes × N_layers ≈ 215 GB` for default settings (256 calib × 2048 × 5120 × 2 × 40). Phase A.X requires sufficient host disk; the spill directory must be writable and have at least this much free space at Phase A entry. If insufficient, Stage 3 raises `RuntimeError` immediately rather than proceeding to a degraded fallback (consistent with the spec's no-silent-degradation policy for Phase C.5).
...
- **Partial-cache case:** if some per-layer caches are present and others missing, the dual-forward re-runs from layer 0 (teacher hidden-state stream is sequential; partial mid-stream re-anchoring would risk dtype/seed drift between resumed and pre-existing caches).
...
**Calibration data:** Nemotron weighted subsets per §2 (D11), identical to Stage 2.5.
...
- **First run:** Teacher is evaluated normally (PPL + zero-shot + generative). Results and param counts are saved to `teacher_eval_cache.json` with a cache key composed of the following inputs (each component is either an HF revision-sha or a `pkg.__version__` string), joined and hashed via SHA-256 (per F-S-H-3):
  ...
  sha256(model_name + revision + tokenizer_revision + dataset_revisions
       ...
       + eval_config_subset)
  ```
  `dataset_revisions` is the JSON-canonicalized mapping `{"wikitext_ppl": "<sha>", "humaneval": "<sha>", "math500": "<sha>"}` serialized with sorted keys before concatenation, ensuring byte-for-byte-equivalent keying across runs. **F-iter4-HIGH-1 limitation:** the lm-eval-managed datasets (HellaSwag, ARC-Challenge) are intentionally **not** pinned in this mapping — `lm-eval` resolves their revisions internally and our `simple_evaluate(...)` call cannot enforce a SHA at load time. The cache key compensates by folding in `lm_eval_version` and a SHA-256 of the lm-eval task config (task list + lm_eval_batch_size); precise per-dataset SHA control for those tasks requires editing lm-eval task YAMLs out-of-band.
...
| D-grape-restart-merge | 1 | One extra merge per restart cycle on entropy restart | 2604.06542 Algorithm 1 (entropy-aware restart): "If all layers frozen and budget not yet met → restart (unfreeze all)" — paper is ambiguous on whether the same iteration runs another merge or the loop re-evaluates entropy first | Spec-original choice: when the restart condition fires (`frozen.clear()`), the same iteration immediately runs argmin layer/pair selection and performs one merge before the entropy gate is re-evaluated on the following iteration. This permits at most one extra merge per restart-cycle beyond the strictest "re-evaluate first" reading. | Allows GRAPE to escape local optima created by premature freezing — without this single extra merge per restart, the loop can stall on a layer whose entropy-state is at the frozen boundary while no other layer can donate. The deviation is bounded (≤ one merge per restart-cycle, and the budget-termination check still gates the merge) and matches the paper's stated goal of restart as an escape mechanism rather than a strict re-evaluation step. |
