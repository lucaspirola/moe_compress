...
| MATH-500 accuracy | `HuggingFaceH4/MATH-500` (revision pinned in run config under `dataset_revisions`); SymPy symbolic equivalence + `\boxed{}` extraction + numeric fallback. Grader: `math_equivalence.py` from Hendrycks et al. 2021 (file SHA pinned in run config under `dataset_revisions["math500_grader"]` and folded into the teacher cache key) | ≤ 3pp absolute drop |
...
- **First run:** Teacher is evaluated normally (PPL + zero-shot + generative). Results and param counts are saved to `teacher_eval_cache.json` with a cache key composed of the following inputs (each component is either an HF revision-sha or a `pkg.__version__` string), joined and hashed via SHA-256 (per F-S-H-3):
  ...
  ```
  Components are joined in the listed order with a fixed `\x1f` (US, ASCII 0x1F) separator so the byte representation is reproducible across implementations and language bindings. `dataset_revisions` is the JSON-canonicalized mapping `{"wikitext_ppl": "<sha>", "humaneval": "<sha>", "math500": "<sha>", "math500_grader": "<grader-file-sha>"}` serialized with sorted keys before concatenation; the `math500_grader` entry pins the `math_equivalence.py` (Hendrycks et al. 2021) implementation hash so swapping a community fork invalidates the cache. **F-iter4-HIGH-1 limitation:** the lm-eval-managed datasets (HellaSwag, ARC-Challenge) are intentionally **not** pinned in this mapping — `lm-eval` resolves their revisions internally and our `simple_evaluate(...)` call cannot enforce a SHA at load time. The cache key compensates by folding in `lm_eval_version` and a SHA-256 of the lm-eval task config (task list + lm_eval_batch_size); precise per-dataset SHA control for those tasks requires editing lm-eval task YAMLs out-of-band.
