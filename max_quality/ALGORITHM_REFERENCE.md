...
9. [Stage 5 — Router Knowledge Distillation](#9-stage-5--router-knowledge-distillation)
...
Stage 1: GRAPE Budget Allocation     → per-layer expert budgets (non-uniform)
   ↓
Stage 2: REAP Score + REAM Merge     → pruned model checkpoint + input covariances
   ↓
Stage 3: Non-Uniform SVD             → factored model (FactoredExperts) + original weight snapshot
   ↓
Stage 4: EoRA Compensation           → widened factors (rank recovery)
   ↓
Stage 5: Router KD                   → recalibrated router weights
   ↓
...
applied to the **compressible pool** (routed expert weights only). Non-compressible parameters (attention, shared experts, embeddings, lm_head, layer norms, router weights) count toward the total denominator but are never modified (except router weights in Stage 5).

**A100 80GB cost estimate:** ~2.5 hours for the full pipeline at 30% reduction.
...
Uniform pruning wastes budget — some layers have highly redundant experts (high pairwise similarity) while others are diverse. GRAPE's +1.40% average accuracy improvement over uniform (and +2.45% peak on Mixtral-8x22B at the 4-expert setting) demonstrates the value of non-uniform allocation.
...
2. **Initialize** each expert as its own cluster. Compute per-layer redundancy `R^l = Σ_{i≠j} D^l_{ij}` (Eq. 11, sum form). Set entropy threshold `Ê = E_0 × (1 − γ)` where `E_0` is the initial cross-layer entropy and `γ=0.1` (Eq. 10).
...
**Per-layer processing (sequential — activations are recomputed after each layer merge):**
...
2. For each centroid, absorb the cheapest (lowest δ_REAM) unassigned non-centroid
...
where `P_i` is a neuron permutation alignment (Hungarian algorithm on weight-based cost matrix `C_wt`) that aligns each child expert's intermediate neurons to the centroid before averaging.
...
Stored in `_stage2_input_covariance.pt` (fp16 storage to avoid bf16 precision loss).
...
#### Phase A: B-covariance Collection

Run the pruned model on fresh calibration data (512 samples, seed_offset=2) to collect per-(layer, expert, matrix) input covariances **B = X_post^T X_post**. These reflect the input distribution the *compressed* model will see.

Per-layer disk spilling bounds peak CPU memory: each layer's covariance (~3–5 GB at fp16) is written to `_stage3_bcov_partial/layer_{idx}.pt` and dropped from memory. Background I/O thread overlaps spill with the next layer's forward pass.
...
k_g = √(R_eff(g) / ω) × T_budget / Σ_{g'} √(R_eff(g') / ω)   (Eq. 6 — rank allocation)
```
...
**When both A (pre-prune covariance from Stage 2) and B (post-prune covariance) are available:**

```
M = W · A_cov · B⁻¹ · L_B
```

where `L_B` is the eigendecomposition-based square root of B, restricted to eigenvalues above a dtype-aware noise floor.

> **Deviation from paper:** This is NOT the pure cross-covariance Theorem 3.2 formula `M = W · (X_pre^T X_post) · B⁻¹ · L_B`, because the cross-covariance `X_pre^T X_post` is not stored. The auto-covariance `A_cov = X_pre^T X_pre` is substituted. The two coincide when pre/post distributions are similar (light pruning). The formula minimizes a hybrid objective: A-cov weights importance of pre-prune directions; B⁻¹ whitens in the post-prune input space.
...
#### Phase D: L-BFGS Block Refinement (Optional)
...
## 9. Stage 5 — Router Knowledge Distillation
...
```
L_RKD = (τ² / N_tokens) × Σ_t KL(softmax(z_T^t / τ) ‖ softmax(z_S^t / τ))
```

where `z_T, z_S ∈ ℝ^{|V|}` are teacher/student vocabulary logits for next-token prediction, and `τ=1.0` is the temperature.
...
**Cost:** ~10 minutes on A100

### What

Evaluates the compressed model against the uncompressed teacher on 5 metrics and enforces hard quality gates.
...
| 2604.01609 | Swift-SVD+: Dynamic Non-Uniform Rank Allocation | 2026 | Stage 3 (planned) |
