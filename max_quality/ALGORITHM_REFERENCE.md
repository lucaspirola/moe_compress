...
- **All subsequent layers (dual-signal OR rule)**: add to `L` if **either** of the following two growth ratios exceeds its threshold:
  ...
  - **MoE-output growth** (secondary, ungated): `max|MoE_l(x)| / max|MoE_{l-1}(x)| > moe_output_growth_ratio`
    where `MoE_l` is the SparseMoeBlock output (post-routing-weighted-sum, pre-residual-add). The MoE branch is **not** dampened by `attn_output_gate` (the gate is only on the attention branch), so this signal carries the raw MA-injection magnitude unattenuated. **`moe_output_growth_ratio = 2.0`** project-specified — set lower than `ma_growth_ratio` because the ungated MoE-output signal has a smaller dynamic range than the gated-attention-plus-MoE residual stream; tuned empirically to maximize recall under the OR rule without picking up noise.
...
After Phase C and the AIMER + sink-token auto-extensions converge, Stage 1 runs a held-out PPL-impact validation pass:

1. **Validation set**: a held-out 100-sample slice of the calibration data not used for Phase A/B (seeded subset, deterministic).
2. **Baseline**: forward pass over the validation set; record token-level NLL and corpus PPL.
3. **For each blacklisted expert `(l, e)`**: zero its `down_proj` output during the forward pass (single-expert ablation hook); record ΔPPL relative to baseline.
4. **For top-`ablation_topk_nonblacklisted` (default 5) non-blacklisted experts per `l ∈ L`** (sorted by `per_expert_max(l, ·)` descending, excluding already-blacklisted): run the same single-expert ablation; record ΔPPL.
5. **Report**: emit `stage1_post_hoc_ablation.json` with `{ablated_id: ΔPPL}` for both groups. **No automatic blacklist mutation** — the operator inspects the report and decides whether the ΔPPL gradient indicates a missed SE.
...
| D-causal-ablation-validation | 1 | Phase F post-hoc causal-ablation report | None — paper validates SE detection via global ablations (Table 3) but not as a per-expert validation step | After Phase C + auto-extensions, run single-expert ablation forward passes on a held-out 100-sample slice. Report ΔPPL for every blacklisted expert + top-5 non-blacklisted candidates per `l ∈ L`. Report-only — operator decides whether to manually extend. Output: `stage1_post_hoc_ablation.json`. | Reviewer's recommended validation step. Cost (~30 min on H200) is small vs Stage 1's total walltime; the report lets us catch missed SEs before downstream ablations spend GPU on a wrong blacklist. |
