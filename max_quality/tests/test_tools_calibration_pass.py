"""Unit tests for ``moe_compress.tools.calibration_pass``.
...
- The ``run_calibration_pass`` convenience wrapper: empty-registration no-op,
  registration-order preservation, and ``per_batch_hooks`` / ``device``
  forwarding.
...
from moe_compress.tools.calibration_pass import (
...
# ---------------------------------------------------------------------------
# ``run_calibration_pass`` convenience wrapper
# ---------------------------------------------------------------------------


def test_run_calibration_pass_empty_registrations_is_noop(tiny_model, monkeypatch):
    """An empty ``registrations`` sequence yields a bare engine — the wrapper
    still runs one forward per batch and installs/leaks nothing."""
    moe_layers = list(iter_moe_layers(tiny_model))
    batches = _make_batches(num_batches=3, batch=1, seq=4)
    ...
    # No error, no hooks, one forward per batch.
    run_calibration_pass(
        tiny_model,
        batches,
        registrations=[],
...
def test_run_calibration_pass_registers_in_sequence_order(tiny_model, monkeypatch):
    """The wrapper must call ``register_accumulator`` in the exact order the
    ``registrations`` sequence provides — registration order is
    byte-identical-critical (fixes both fan-out and per-batch handler order)."""
    moe_layers = list(iter_moe_layers(tiny_model))
    batches = _make_batches(num_batches=1, batch=1, seq=4)
    ...
    # Spy on register_accumulator to record the call order.
    recorded: list[str] = []
    original_register = CalibrationEngine.register_accumulator

    def _spy_register(self, name, accumulator, hook_spec):
        recorded.append(name)
        return original_register(self, name, accumulator, hook_spec)

    monkeypatch.setattr(CalibrationEngine, "register_accumulator", _spy_register)
    ...
    assert recorded == ["c", "a", "b"]


def test_run_calibration_pass_forwards_per_batch_hooks_and_device(
    tiny_model, monkeypatch
):
    """The wrapper must forward ``per_batch_hooks`` and ``device`` straight
    through to ``CalibrationEngine.run``."""
    moe_layers = list(iter_moe_layers(tiny_model))
    batches = _make_batches(num_batches=2, batch=1, seq=4)

    # Spy on CalibrationEngine.run to capture the forwarded kwargs.
    seen: dict[str, object] = {}
    original_run = CalibrationEngine.run

    def _spy_run(self, model, batches, **kwargs):
        seen["device"] = kwargs.get("device")
        seen["per_batch_hooks"] = kwargs.get("per_batch_hooks")
        return original_run(self, model, batches, **kwargs)

    monkeypatch.setattr(CalibrationEngine, "run", _spy_run)

    log: list[tuple[str, int]] = []
    rec = _RecorderAcc()  # at least one expert callback keeps the path lit
    registrations = [
        (
            "rec",
            rec,
    ...
    run_calibration_pass(
        ...
        per_batch_hooks=hooks,
    )

    # device + per_batch_hooks reached run() unchanged.
    assert seen["device"] == torch.device("cpu")
    assert seen["per_batch_hooks"] is hooks
    # The hooks actually fired, in order, once per batch.
    assert log == [("a", 0), ("b", 0), ("a", 1), ("b", 1)]
