...
# ---------------------------------------------------------------------------
# Batched pass — many-pair cost matrix exercises the tiled SwiGLU batching
# ---------------------------------------------------------------------------


def test_output_cost_multi_noncentroid_batched_path_is_correct():
    """Many-pair cost matrix — exercises the tiled batched-SwiGLU pass.

    ``_output_space_cost``'s serial per-pair SwiGLU forward was replaced by a
    tiled batched matmul over the pair axis (the per-pair Hungarian permutation
    solve stays serial — it does not vectorize). With several non-centroids and
    centroids the matrix spans multiple cells; this test recomputes EVERY cell
    with the independent ``_independent_output_cost`` reference (which uses the
    per-pair ``_swiglu_forward``), so a batching bug — a mis-indexed pair, a
    wrong stack/broadcast axis, a clobbered cell — surfaces as a wrong entry.
    """
    import torch.nn as nn
    from moe_compress.utils.model_io import MoELayerRef

    hidden, d_int, n_exp, top_k = 12, 7, 8, 3
    ...
    class _Experts(nn.Module):
        def __init__(self):
            ...
            self.gate_up_proj = nn.Parameter(
                torch.randn(n_exp, 2 * d_int, hidden) * 0.1
            )
            self.down_proj = nn.Parameter(torch.randn(n_exp, hidden, d_int) * 0.1)
    ...
    class _Router(nn.Module):
        def __init__(self):
            ...
            self.top_k = top_k
            self.hidden_dim = hidden
            self.weight = nn.Parameter(torch.randn(n_exp, hidden) * 0.5)
    ...
    freq = {e: (e + 2) * 4 for e in range(n_exp)}  # distinct, non-uniform
    # 256 tokens over 8 experts at top_k=3 → every expert (incl. each
    # non-centroid) is routed, so no row hits the no-token fallback.
    x = torch.randn(256, hidden)

    noncentroid_ids = [0, 1, 2]
    centroid_ids = [3, 4, 5, 6, 7]
    n_nc, n_c = len(noncentroid_ids), len(centroid_ids)
    cheap = np.random.rand(n_nc, n_c)
    ...
    cost = _output_space_cost(
        ...
        cheap_cost=cheap,
        ream_acc=None, perm_cache=None,
        topk=n_c,  # every centroid is a candidate → every cell finite
        freq=freq, layer_inputs=x, token_cap=1024,
    )

    assert cost.shape == (n_nc, n_c)
    assert np.isfinite(cost).all(), (
        "every cell should be finite (all centroids are candidates and every "
        "non-centroid receives tokens); a +inf means a pair task was dropped"
    )
    for ci, m_id in enumerate(noncentroid_ids):
        for cj, c_id in enumerate(centroid_ids):
            expected = _independent_output_cost(layer_ref, m_id, c_id, x, freq)
            assert cost[ci, cj] == pytest.approx(expected, rel=1e-5, abs=1e-7), (
                f"cell ({ci},{cj}) m={m_id} c={c_id}: threaded cost "
                f"{cost[ci, cj]} != independent recompute {expected}"
            )
    # Distinct random experts → every merge genuinely costs something.
    assert (cost > 0).all()
