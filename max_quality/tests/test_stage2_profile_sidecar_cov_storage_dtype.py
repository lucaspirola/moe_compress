...
def _build_payload(cov_storage_dtype: str) -> Stage2ProfilePayloadV4:
    ...
    return Stage2ProfilePayloadV4(
        ...
        gate_gram=torch.zeros((n_layers, n_experts, n_experts), dtype=torch.float64),
...
def test_matching_dtype_loads_ok(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    loaded = load_stage2_profile_v4(
...
def test_mismatched_dtype_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_cov_storage_dtype="bfloat16")
...
def test_mismatched_dtype_fp32(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_cov_storage_dtype="float32")


def test_bfloat16_sidecar_roundtrips(tmp_path):
    """bfloat16 sidecar dtype survives the round-trip and validates."""
    jsonl = _jsonl(tmp_path)
    save_stage2_profile_v4(_build_payload("bfloat16"), jsonl)
    loaded = load_stage2_profile_v4(
...
def test_cross_validation_n_layers_mismatch(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_n_layers=99)
...
def test_cross_validation_n_experts_mismatch(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_n_experts=99)
...
def test_cross_validation_top_k_mismatch(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_top_k=99)
...
def test_cross_validation_model_hash_mismatch(tmp_path):
    ...
    save_stage2_profile_v4(_build_payload("float16"), jsonl)
    with pytest.raises(ValueError, match="Delete the sidecar to regenerate"):
        load_stage2_profile_v4(jsonl, expected_model_hash="otherhash")
